...
class NSM_Project_Query{

	/**
	 * The args to pass to the nsm_get_projects() query
	 *
	 * @var array
	 * @since 1.0.0
	 */
	...
	/**
	 * The projects found based on the criteria set
	 *
	 * @var array
	 * @since 1.0.0
	 */
	...
	public function __construct( $args = array() ){
		...
		if( isset( $GLOBALS['wp_query'] ) && isset( $GLOBALS['wp_query']->query ) && is_post_type_archive( 'project' ) ){
	...
	public function get_projects() {
		...
		if ( $query->have_posts() ) {
			...
			$this->projects['max_num_pages'] = $query->max_num_pages;
			$this->projects['found_posts'] 	 = $query->found_posts;
		}
	 ...
	 * @return void
	 */
	private function project_type(){
		if ( empty( $this->args['project_type'] ) ) {
			return;
		}

		$this->__set( 'tax_query', array(
			'taxonomy'  	=> 'project_type',
			'field' 		=> 'term_id',
			'terms'			=>	$this->args['project_type']
		) );
		$this->__unset( 'project_type' );
