...
class NSM_Job_Query{
	...
	/**
	 * The args to pass to the nsm_get_jobs() query
	 *
	 * @var array
	 * @since 1.0.0
	 */
	...
	/**
	 * The jobs found based on the criteria set
	 *
	 * @var array
	 * @since 1.0.0
	 */
	...
	public function __construct( $args = array() ){
		...
		if( isset( $GLOBALS['wp_query'] ) && isset( $GLOBALS['wp_query']->query ) && is_post_type_archive( 'job' ) ){
	...
	public function get_jobs() {
		...
		if ( $query->have_posts() ) {
			...
			$this->jobs['max_num_pages'] = $query->max_num_pages;
			$this->jobs['found_posts'] 	 = $query->found_posts;
		}
	 ...
	 * @return void
	 */
	private function job_location(){
		if ( empty( $this->args['job_location'] ) ) {
			return;
		}
		...
		$this->__set( 'tax_query', array(
			...
			'field' 		=> 'term_id',
			'terms'			=>	$this->args['job_location']
		) );
		$this->__unset( 'job_location' );
